...
import { computeDivergence, DivergenceReading } from './services/DivergenceAnalyzer';
...
const App: React.FC = () => {
  ...
  const [divergence, setDivergence]         = useState<DivergenceReading | null>(null);
  const [divergenceHistory, setDivergenceHistory] = useState<DivergenceReading[]>([]);
  ...
  const handleLiveTick = useCallback((tick: NormalizedMarketTick) => {
    try {
      ...
      const div = computeDivergence(result.signals, tick.processing_timestamp);
      setDivergence(div);
      setDivergenceHistory(prev => [...prev, div].slice(-60));
  ...
  useEffect(() => {
    ...
    simTimerRef.current = setInterval(() => {
      ...
      if (lastResult && lastTick && lastPoint) {
        ...
        const div = computeDivergence(lastResult.signals, lastTick.processing_timestamp);
        setDivergence(div);
        setDivergenceHistory(prev => [...prev, div].slice(-60));
        setTimelineData(prev => [...prev, {
    ...
    <div className="min-h-screen flex flex-col p-4 gap-4 max-w-[1920px] mx-auto overflow-hidden">
      ...
      <main className="flex-1 grid grid-cols-12 gap-4 h-[calc(100vh-200px)] overflow-hidden">
        ...
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar">
          ...
          <DivergencePanel current={divergence} history={divergenceHistory} />
        </div>
      ...
      <div className="px-6 pb-6 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-12 gap-4">
          <div className="col-span-12 lg:col-span-6">
            <CounterfactualLab historicalPoints={historicalPoints} historyLoader={historyLoader} />
          </div>
          <div className="col-span-12 lg:col-span-6">
            <BlindSpots />
          </div>

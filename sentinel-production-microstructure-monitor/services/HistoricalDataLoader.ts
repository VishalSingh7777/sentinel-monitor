...
// Named historical windows for regime comparison. All fetched from the same
// free, keyless Binance klines endpoint already used by loadCovidCrash.
...
export const REGIME_PRESETS: RegimePreset[] = [
  { label: 'COVID Crash (Mar 2020)',   startTime: Date.UTC(2020, 2, 11), endTime: Date.UTC(2020, 2, 14, 23, 59, 59) },
  { label: 'FTX Collapse (Nov 2022)',  startTime: Date.UTC(2022, 10, 6), endTime: Date.UTC(2022, 10, 10, 23, 59, 59) },
  { label: 'Calm Baseline (Jan 2024)', startTime: Date.UTC(2024, 0, 8),  endTime: Date.UTC(2024, 0, 11, 23, 59, 59) },
];
...
export class HistoricalDataLoader {
  ...
  async loadCovidCrash(): Promise<HistoricalDataPoint[]> {
    ...
    try {
      ...
      const processedData = allKlines.map(k => this.processKline(k));
  ...
  // Shared kline → HistoricalDataPoint conversion, used by both loadCovidCrash
  // and the generic loadWindow (regime comparison).
  private processKline(k: any): HistoricalDataPoint {
  ...
  // Generic fetch for any [startTime, endTime) window — used for regime
  // comparison (FTX collapse, a calm baseline week, etc.) without touching
  // the COVID-specific caching/behavior above.
  async loadWindow(startTime: number, endTime: number): Promise<HistoricalDataPoint[]> {
    let allKlines: any[] = [];
    ...
    while (currentStart < endTime) {
      ...
      if (!response.ok) throw new Error('Failed to fetch historical window');
      const data = await response.json();
      if (data.length === 0) break;
      allKlines    = [...allKlines, ...data];
      currentStart = data[data.length - 1][0] + 60000;
    ...
    return allKlines.map(k => this.processKline(k));
  }
